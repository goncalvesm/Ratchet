...
class TopicTest extends TestCase {
    ...
    public function testBroadcast() {
        ...
        $first = $this->getMockBuilder('Ratchet\\Wamp\\WampConnection')
            ->onlyMethods(['send'])
            ->setConstructorArgs([$this->createMock('\\Ratchet\\ConnectionInterface')])
            ->getMock();
        $second = $this->getMockBuilder('Ratchet\\Wamp\\WampConnection')
            ->onlyMethods(['send'])
            ->setConstructorArgs([$this->createMock('\\Ratchet\\ConnectionInterface')])
            ->getMock();
    ...
    public function testBroadcastWithExclude() {
        ...
        $second = $this->getMockBuilder('Ratchet\\Wamp\\WampConnection')
            ...
            ->setConstructorArgs([$this->createMock('\\Ratchet\\ConnectionInterface')])
            ->getMock();
        $third = $this->getMockBuilder('Ratchet\\Wamp\\WampConnection')
            ->onlyMethods(['send'])
            ->setConstructorArgs([$this->createMock('\\Ratchet\\ConnectionInterface')])
            ->getMock();
